...
FAIRNESS_CLASSIFY_PROMPT = """당신은 금융 감독원의 약관 심사 전문가입니다.

[작업]
다음 약관 조항이 '공정'한지 '불공정'한지 판단하세요.
...
[판단 기준]
1.  **'불공정' 판단 기준:**
    * 고객의 법률상 권리(항변권, 이의제기권 등)를 부당하게 제한하는가?
    * 사업자의 책임을 일방적으로 면제하거나 고객에게 전가하는가?
    * 고지 절차 없이 일방적으로 서비스를 변경/중단/해지할 수 있는가?
    * 해지 사유가 "기타 부득이한 사유"처럼 포괄적이거나 자의적인가?
    * 환급을 부당하게 제한하거나 과도한 위약금을 부과하는가?

2.  **[주의] '공정' 판단 기준:**
    * 조항에 '간주 동의' (예: "이의가 없으면 동의한 것으로 본다")가 포함되어 있더라도, **고객에게 충분한 고지**와 **명확한 이의제기 기간(예: 1개월)**을 부여했다면 '공정'으로 판단합니다.
    * 조항에 '고객 책임'이 언급되더라도, 그것이 **고객의 고의/과실**에 대한 책임이거나 **법령상 고객의 의무(예: 주소 변경 통지)**를 명시한 것이라면 '공정'으로 판단합니다.
    * 사업자의 책임을 면제하는 것처럼 보여도, 그것이 **'천재지변' 등 불가항력**이거나 **'회원의 고의/중과실이 없는 경우'** 등 예외를 명시했다면 '공정'일 수 있습니다.

[출력 규칙]
1.  첫 줄에 "공정" 또는 "불공정" 중 하나만 출력하세요.
2.  둘째 줄에 확신도를 0.0~1.0 사이 소수점 둘째자리 숫자로 출력하세요. (예: 0.85)
...
UNFAIR_TYPE_PROMPT = """당신은 금융 약관 심사 전문가입니다.
...
[작업]
1.  아래 [불공정 유형] 중 가장 적절한 것을 **단 하나만** 선택하세요.
2.  [중요 분류 규칙]을 반드시 참고하여 신중하게 판단하세요.
...
8.  **기타 불공정 약관**
    ...
    * (예: **부당한 재판관할 합의(법원 지정)**, 고객 의사 확인 없는 계약 자동 연장, 인지세 고객 전가, 개인정보 포괄적 수집/활용 동의, 약관의 불리한 소급 적용)

[중요 분류 규칙]
* **(3번 vs 7번)** '7. 면책·책임 전가'는 사업자가 져야 할 책임을 고객에게 넘기거나(예: 제휴사 문제, 시스템 오류) 책임을 아예 지지 않는(예: 부정사용 면책) 조항입니다. 이는 고객의 이의제기 권리 등을 제한하는 '3. 고객 권리 제한'과 명확히 구별됩니다.
* **(3번 vs 8번)** '8. 기타 불공정 약관'은 주로 **'재판관할'**이나 **'관할 법원'**을 사업자에게 유리하게 지정하는 조항을 포함합니다. 조항이 **법원이나 재판**을 언급하면 '8번'일 가능성이 높으니 '3번'으로 잘못 분류하지 않도록 극히 주의해야 합니다.
* **(3번 vs 4번)** '4. 통지·고지 부적절'은 '간주 동의'의 **절차(방식)**가 부적절한(예: 앱푸시, 홈페이지 게시) 경우입니다. '간주 동의'의 **내용** 자체가 고객 권리를 제한하는 경우 '3번'입니다.
